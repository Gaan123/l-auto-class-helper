...
//    }
    ...
    protected function bindClass(string $basePathAbstractClass,string $basePathConcreteClass,array $extra=[]): bool
    {
        ...
        if (count($dynBind))$this->dynamicBind($dynBind);
        return true;
    }
...
//    }
    ...
    protected function getClasses(string $path):array
    {
        ...
        foreach ($finder as $file) {
            if ($this->getNameSpaceFromFile($file))$classes[]=$this->getNameSpaceFromFile($file).'\\'.str_replace('/','\\',explode('.',$file->getRelativePathname()))[0];;
        }
        return $classes;
    }
